...
- (CGFloat)heightForContent {
    ...
    return [self.class heightForString:self.text width:self.frame.size.width font:self.font];
}

- (void)updateHeightForContent {
    CGRect frame = self.frame;
    frame.size.height = self.heightForContent;
    self.frame = frame;
}
...
- (CGFloat)widthForContent {
    ...
    return [self.class widthForString:self.text height:self.frame.size.height font:self.font];
}

- (void)updateWidthForContent {
    CGRect frame = self.frame;
    frame.size.width = self.widthForContent;
    self.frame = frame;
}
