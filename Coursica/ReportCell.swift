...
class ReportCell: UITableViewCell {
    ...
    func layoutTitleLabel(title: String) {
        ...
        constrain(titleLabel, block: {title in
    ...
    func layoutResponseViews(legend: Bool) {
        ...
        constrain([titleLabel] + responseViews, replace: ConstraintGroup(), block: {views in
            let background = views.last!.superview!
            let title = views.first!
            let responseViews: [LayoutProxy] = Array(views[1..<views.count])
            for (index, view) in responseViews.enumerate() {
        ...
        let bottomSpacing = constrain(responseViews.last!, roundedBackgroundView, block: {last, background in
        ...
        if legend {
            ...
            constrain(legendView, responseViews.last!, replace: bottomSpacing, block: {legend, lastResponseView in
